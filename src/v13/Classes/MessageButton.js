...
const { verifyString, resolveString } = require('discord.js').Util;
...
class MessageButton extends BaseMessageComponent {
    ...
    setup(data) {
        ...
        this.label = ('label' in data && data.label) ? verifyString ? verifyString(data.label) : resolveString(data.label) : undefined;
        ...
        if ('url' in data && data.url)
            this.url = verifyString ? verifyString(data.url) : resolveString(data.url)
    ...
    setLabel(label) {
        label = verifyString ? verifyString(label) : resolveString(label);
        this.label = label;
        return this;
    }
    ...
    setURL(url) {
        this.url = verifyString ? verifyString(url) : resolveString(url);
        return this;
    }

    setID(id) {
        this.custom_id = verifyString ? verifyString(id) : resolveString(id);
        return this;
    }

    setEmoji(emoji, animated) {
        if (!emoji) return this;
        if (isEmoji(emoji) === true) this.emoji = { name: verifyString ? verifyString(emoji) : resolveString(emoji) }
        else if (emoji.id) this.emoji = { id: emoji.id }
        else if (verifyString ? verifyString(emoji).length > 0 : resolveString(emoji).length > 0) this.emoji = { id: verifyString ? verifyString(emoji) : resolveString(emoji) }
